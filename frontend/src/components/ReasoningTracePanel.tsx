...
const AGENT_META: Record<string, { label: string; icon: any; color: string }> = {
  orchestrator: { label: "Orchestrator", icon: Workflow, color: "text-brand-400" },
  query_agent: { label: "Query Agent", icon: Database, color: "text-emerald-400" },
  analysis_agent: { label: "Analysis Agent", icon: LineChart, color: "text-gold-400" },
...
function StatusIcon({ status }: { status: string }) {
  if (status === "running") return <Loader2 className="h-3.5 w-3.5 animate-spin text-slate-400" />;
...
function StepDetail({ event }: { event: AgentEvent }) {
  const d = event.data || {};
  return (
    <div className="mt-1.5 ml-6 space-y-1.5 text-xs text-slate-400">
      {d.sql && (
        <pre className="overflow-x-auto rounded-md bg-ink-900 border border-ink-600 p-2 font-mono text-[11px] text-emerald-300">
          {d.sql}
        </pre>
      )}
      {typeof d.row_count === "number" && (
        <div>
          Rows returned: <span className="text-slate-200">{d.row_count}</span>
          {d.filters_summary ? <> · Filters: <span className="text-slate-200">{d.filters_summary}</span></> : null}
        </div>
      )}
      ...
      {typeof d.numbers_checked === "number" && (
        <div>
          Figures verified: <span className="text-slate-200">{d.verified_count}/{d.numbers_checked}</span>
          {" · "}confidence:{" "}
          <span className={
            d.confidence === "high" ? "text-emerald-400"
              : d.confidence === "medium" ? "text-gold-400" : "text-rose-400"
          }>{d.confidence}</span>
...
export default function ReasoningTracePanel({
  ...
  return (
    <div className="mt-3 rounded-lg border border-ink-600 bg-ink-800/60">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex w-full items-center gap-2 px-3 py-2 text-left text-xs font-medium text-slate-300 hover:text-white"
      >
        {open ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
        <Workflow className="h-4 w-4 text-brand-400" />
        Reasoning trace
        <span className="ml-1 rounded-full bg-ink-600 px-2 py-0.5 text-[10px] text-slate-400">
        ...
        <div className="space-y-2 px-3 pb-3">
          {visible.map((event, i) => {
            const meta = AGENT_META[event.agent] || {
              label: event.agent, icon: Workflow, color: "text-slate-400",
            };
